...
$query="DELETE FROM drugs_used WHERE id_serv=".mysql_real_escape_string($_REQUEST["id"]);
...
$query="DELETE FROM pat_serv_med WHERE id=".mysql_real_escape_string($_REQUEST["id"]);
$result=mysql_query($query);
if($result==false)
{
	header("Query: ".$query,true);
