...
if(!isset($_REQUEST["interval"]))
{
	...
	while($date<=$end_date)
	{
		$query="SELECT COUNT(DISTINCT id_pat) as num_pat FROM pat_serv_med WHERE serv_date='".$date->format('Y-m-d')."'";
...
else
{
	switch($_REQUEST["interval"])
	{
		...
		case "month":
			...
			if($next_date==$end_date)
			{
				$query="SELECT COUNT(DISTINCT id_pat) as num_pat FROM pat_serv_med WHERE serv_date>='".$date->format('Y-m-d')."' AND serv_date<='".$next_date->format('Y-m-d')."'";
				$result=mysql_query($query);
				header("Query$n: ".$query);
				$n++;
				if($result==false)
...
echo $s;
$query="SELECT COUNT(DISTINCT id_pat) as num_pat FROM pat_serv_med WHERE serv_date>='".$begin_date->format('Y-m-d')."' AND serv_date<='".$end_date->format('Y-m-d')."'";
