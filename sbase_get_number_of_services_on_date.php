...
if(!isset($_REQUEST["interval"]))
{
	...
	while($date<=$end_date)
	{
		$query="SELECT COUNT(id) as num_serv FROM pat_serv_med WHERE serv_date='".$date->format('Y-m-d')."'";
		...
		{
			$s.=($date->getTimestamp()*1000).'|'.$row['num_serv'].';';
		}
		$date->add($interval);
	}
}
else
{
	switch($_REQUEST["interval"])
	{
		...
		case "month":
			...
			if($next_date==$end_date)
			{
				$query="SELECT COUNT(id) as num_serv FROM pat_serv_med WHERE serv_date>='".$date->format('Y-m-d')."' AND serv_date<='".$next_date->format('Y-m-d')."'";
				$result=mysql_query($query);
				header("Query$n: ".$query);
				$n++;
				if($result==false)
				...
				if($row=mysql_fetch_array($result))
				{
					$s.=($next_date->getTimestamp()*1000).'|'.$row['num_serv'].';';
				}
			}
			else
...
$query="SELECT COUNT(pat_serv_med.id) as num_serv,serv_date FROM pat_serv_med WHERE pat_serv_med.serv_date>='".mysql_real_escape_string($_REQUEST["date"])."' AND pat_serv_med.serv_date<='".mysql_real_escape_string($_REQUEST["to_date"])."' GROUP BY serv_date";

$s=substr_replace($s ,"",-1);
echo $s;
$query="SELECT COUNT(id) as num_serv FROM pat_serv_med WHERE serv_date>='".$begin_date->format('Y-m-d')."' AND serv_date<='".$end_date->format('Y-m-d')."'";
