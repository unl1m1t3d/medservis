...
if(!isset($_REQUEST["interval"]))
{
	...
	while($date<=$end_date)
	{
		$query="SELECT IFNULL(SUM(sum),0) FROM charges WHERE date_of_charge='".$date->format('Y-m-d')."'";
...
else
{
	switch($_REQUEST["interval"])
	{
		...
		case "month":
			...
			if($next_date==$end_date)
			{
				$query="SELECT IFNULL(SUM(sum),0) FROM charges WHERE date_of_charge>='".$date->format('Y-m-d')."' AND date_of_charge<='".$next_date->format('Y-m-d')."'";
				$result=mysql_query($query);
				header("Query$n: ".$query);
				$n++;
				if($result==false)
...
echo $s;
$query="SELECT IFNULL(SUM(sum),0) FROM charges WHERE date_of_charge>='".$begin_date->format('Y-m-d')."' AND date_of_charge<='".$end_date->format('Y-m-d')."'";
